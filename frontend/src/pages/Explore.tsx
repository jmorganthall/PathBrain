...
// ── Levers: what moving one setting does, measured in the ring ─────────────────────
//
// The matched-pairs card above is the observational version of this question: profiles
// that happen to differ in one lever, measured on different nights. This is the controlled
// version — every row is paired, interleaved, same-weather duel evidence — read beside
// what fq_codel theory predicts on an unsaturated link. A prediction that fails is the
// interesting row.
...
function LeverLedgerCard({ book }: { book: LeverLedger | null }) {
  const navigate = useNavigate();
      ...
      {!book ? (
        ...
        <>
          <Typography variant="caption" color="text.secondary" sx={{ display: "block", mb: 1 }}>
            ...
            A direction is stated only past {book.min_rounds} rounds and p &lt; {book.alpha}. Set the duel&apos;s
            &quot;Who challenges&quot; to <b>Levers</b> to fill this table on purpose.
          </Typography>
...
export default function Explore() {
  ...
  // The lever ledger — controlled single-lever evidence off the duel ledger. Cheap, so it
  // loads with the page rather than with the landscape.
  const [leverBook, setLeverBook] = useState<LeverLedger | null>(null);
  useEffect(() => {
    api.leverLedger().then(setLeverBook).catch(() => setLeverBook(null));
  }, []);
    ...
    <Box>
      ...
      {data && !data.reason && (
        <>
          ...
          <LeverLedgerCard book={leverBook} />
