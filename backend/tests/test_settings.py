...
def test_reanchor_forks_a_new_version_and_makes_it_current(client, monkeypatch):
    ...
    assert cur["definition"]["overall"]["metrics"] == ["fcp", "lcp", "network_stall_all"]
...
def test_overall_follows_the_crown_metrics_not_the_smoothness_axis(client):
    ...
    # (fcp/lcp/stall_time). The Overall follows the crown metrics, so B ranks above A — even
...
def test_crown_rewards_balance_over_specialism():
    ...
    # SPEC is fastest on fcp/lcp but *slowest* on stall_time; BAL is consistently 2nd on all.
    profiles = [
        {"metrics": {"fcp": 10, "lcp": 10, "stall_time": 900}},   # SPEC: aces two, worst on one
        {"metrics": {"fcp": 20, "lcp": 20, "stall_time": 100}},   # BAL: 2nd on everything
        {"metrics": {"fcp": 30, "lcp": 30, "stall_time": 200}},
        {"metrics": {"fcp": 40, "lcp": 40, "stall_time": 300}},
        {"metrics": {"fcp": 50, "lcp": 50, "stall_time": 400}},
    ]
    ...
    # …but is far worse on stall_time…
    assert spec["norm"]["stall_time"] < bal["norm"]["stall_time"]
...
def test_crown_skips_run_missing_required_metric(client):
    t0 = datetime.now(timezone.utc).replace(tzinfo=None)
    # A run missing a required *raw* crown metric (only fcp captured; no lcp/stall_time) can't
...
def test_percentile_norm_equalizes_spread_so_no_metric_dominates():
    ...
    wide = [10, 500, 1000]      # stall_time-like: big magnitudes
...
def test_normalized_crown_is_monotonic_and_threshold_free():
    ...
    profiles = [
        {"metrics": {"fcp": 200, "lcp": 240, "stall_time": 100}},
        {"metrics": {"fcp": 300, "lcp": 360, "stall_time": 400}},
    ]
    field = _crown_field_values(profiles, metrics)
...
def test_normalized_crown_missing_required_metric_is_none():
    ...
    assert res["overall"] is None  # lcp/stall_time absent → no corner
...
def test_custom_crown_corners_selected_betterments(client):
    ...
    for i in range(3):
        _seed_run("xcustom0000x", 80, t0 - timedelta(minutes=120 - i), iterations=6,
                  crown_subscores={"fcp": 100, "lcp": 50, "stall_time": 80, "load_event": 50})
    for i in range(3):
        _seed_run("ycustom0000x", 80, t0 - timedelta(minutes=60 - i), iterations=6,
                  crown_subscores={"fcp": 50, "lcp": 100, "stall_time": 80, "load_event": 100})
...
def _prof(fp: str, overall: float, iqr: float = 0.0, count: int = 16, iterations: int = 30) -> dict:
    """A minimal confident-profile dict for the pure crown selector: an Overall median with a
    symmetric IQR band and a run ``count`` (n) — the tie test uses the SE of the median (IQR/√n),
    so ``count`` matters as much as the spread."""
    return {
        "fingerprint": fp,
        "overall": overall,
...
def test_clearly_better_uses_standard_error_of_the_median():
    # The tie test is gap > max(floor, σ · pooled-SE), SE = IQR/√n. σ=2 here.
    tight_hi = _prof("hi", 90.0, iqr=0.0)
    tight_lo = _prof("lo", 70.0, iqr=0.0)
    assert _clearly_better(tight_hi, tight_lo, 0.5, 2.0) is True  # 20-pt lead, SE≈0 → real
    # A jittery, thin profile: iqr 12 over n=9 → SE 4, pooled 4, band 2·4=8. gap 1 < 8 → tie.
    wide = _prof("wide", 89.0, iqr=12.0, count=9)
    assert _clearly_better(tight_hi, wide, 0.5, 2.0) is False
    near = _prof("near", 89.7, iqr=0.0)
    assert _clearly_better(tight_hi, near, 0.5, 2.0) is False  # gap 0.3 < 0.5 floor


def test_sample_size_can_break_a_tie():
    # THE point of the change: two profiles reliably ~2 apart with the SAME spread. Thin samples
    # can't separate them; enough runs shrink the SE until the lead is significant.
    a_thin = _prof("a", 82.0, iqr=6.0, count=9)     # SE 2.0
    b_thin = _prof("b", 80.0, iqr=6.0, count=9)     # SE 2.0 → pooled 2.83, band 5.66 → gap 2 < 5.66
    assert _clearly_better(a_thin, b_thin, 0.5, 2.0) is False  # thin → still a tie
    a_fat = _prof("a", 82.0, iqr=6.0, count=225)    # SE 0.4
    b_fat = _prof("b", 80.0, iqr=6.0, count=225)    # SE 0.4 → pooled 0.57, band 1.13 → gap 2 > 1.13
    assert _clearly_better(a_fat, b_fat, 0.5, 2.0) is True  # more runs → the lead separates


def test_crown_follows_highest_median_even_by_a_hair():
    # jittery leads by a hair (96 vs 95) with a wide, thin band; steady is a touch lower but
    # rock-steady. The winner is the highest median, period — jittery is crowned even though the
    # lead is inside the noise. Both are still returned as co-leaders (informational).
    steady = _prof("steady", 95.0, iqr=0.0)
    jittery = _prof("jittery", 96.0, iqr=12.0, count=9)  # SE 4 → 1 < 2·4 → within noise
    best, co = _select_crown([steady, jittery], min_margin=0.5, sigma=2.0)
...
def test_crown_ignores_the_active_profile_no_stickiness():
    ...
    best, co = _select_crown([deployed_lo, higher], min_margin=0.5, sigma=2.0)
...
def test_crown_clear_winner_has_no_co_leaders():
    ...
    best, co = _select_crown([lo, hi], min_margin=0.5, sigma=2.0)
...
def test_profiles_endpoint_flags_co_leaders_on_a_tie(client, monkeypatch):
    ...
    # Crown-lead-vs-noise readout: identical twins → gap ~0 → not a clear lead, ≥1 co-leader.
    cc = body["crown_confidence"]
    assert cc is not None
    assert cc["clear_lead"] is False
    assert cc["co_leader_count"] >= 1
    assert cc["gap_to_runner_up"] is not None and cc["noise_threshold"] is not None
    assert cc["overall"] is not None and cc["sigma"] == 2.0
...
def test_optimizer_export_has_settings_runs_and_raw_metrics(client):
    ...
    # crown_raw is positional by crown order → fcp=210, lcp=250, network_stall_all=140.0.
    assert sample["metrics"]["fcp"] == 210.0 and sample["metrics"]["network_stall_all"] == 140.0
...
def test_weather_adjusted_overall_strips_setup_weather(client):
    # Two profiles with identical raw FCP/LCP/network_stall_all, so their raw Overall ties. One ran
    ...
    t0 = datetime.now(timezone.utc).replace(tzinfo=None)
    crown = _crown_metrics()  # [fcp, lcp, network_stall_all]
    raw = tuple({"fcp": 500.0, "lcp": 800.0, "network_stall_all": 300.0}[m] for m in crown)
    for i in range(6):
