...
def test_has_latest_metrics_helper():
    assert has_latest_metrics({"fcp": 400, "lcp": 600, "ttfb": 200}) is True
    assert has_latest_metrics({"ttfb": 200, "render": 1500}) is False  # no paint
    assert has_latest_metrics({}) is False
    assert has_latest_metrics(None) is False


def test_history_flags_legacy(client):
    current = _seed({"fcp": 400.0, "lcp": 600.0})
...
def test_rolling_excludes_legacy(client):
    _seed({"fcp": 400.0, "lcp": 600.0}, sops=90.0)
